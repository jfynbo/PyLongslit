"""
This is a python program to extract 1d-spectra from reduced 
observation frames (both Science and Standard Star).
...
def extract_1d_spec(standard_star = False):
    ...
    if not standard_star:
        ...
        _.writeto(DATAPATH/(STDIMAGE.stem+".skysub.fits"), overwrite=True)
        _ = fits.PrimaryHDU(data=data_variance, header=hdr)
        _.data = _.data.astype('float32')
        _.writeto(DATAPATH/(STDIMAGE.stem+".variance.fits"), overwrite=True)
        _ = fits.PrimaryHDU(data=data_sky, header=hdr)
        _.data = _.data.astype('float32')
        _.writeto(DATAPATH/(STDIMAGE.stem+".sky.fits"), overwrite=True)
    ...
    if not standard_star:
        ...
        hduout.writeto(STDIMAGE.stem+".ms_1d.fits", overwrite=True)
        print(" ")
        print("** Wrote output file '%s.ms_1d.fits' ." % STDIMAGE.stem)
        print(" ") 

        #Also write spectrum to an ascii-file
        df_frame = {'wave':wavelength, 'optflux':out1, 'sumflux':out2, 'sky':out3, 'opt_sigma':out4}
        df = pd.DataFrame(df_frame,dtype='float32')
        df.to_csv(STDIMAGE.stem+'.ms_1d.dat', header=None, index=None, sep=' ')
        print(" ")
        print("** Wrote output file '%s.ms_1d.dat' ." % STDIMAGE.stem)
        print(" ")
